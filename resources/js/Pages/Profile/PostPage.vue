<template>
    ...
    <div class="mt-32">
        <div v-if="posts.length > 0" class="flex justify-center" >
            <div v-for="(post,index) in posts" :key="post.id" class=" border border-solid border-gray-200 rounded-md  py-3 my-3 mt-5 md:mt-0 w-full md:w-1/2">
                    ...
                    onSuccess: (page) => {
                        ...
                        if(response.success){
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    // self.$inertia.visit(self.route('profile',self.logged_in_users_details.id))
                                }
                            });
                        }else{
                            Swal.fire({
